#!/usr/bin/env node
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// eigo-kore ペットキャラクター画像 一括生成ツール（Leonardo.ai版）
// leonardo-ai-image-gen スキルのテンプレートを流用
//
// 使い方:
//   $env:LEONARDO_API_KEY = [Environment]::GetEnvironmentVariable("LEONARDO_API_KEY","User")
//   node generate_leonardo.js --count 2   # サンプル2枚だけ（未生成分のみ）
//   node generate_leonardo.js --all       # 20枚まとめて生成
//   node generate_leonardo.js --all --force
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const fs = require('fs');
const path = require('path');
const { getAllPetCombos, buildPrompt } = require('./prompt_builder');
...
async function main() {
  ...
  const allCombos = getAllPetCombos();
  console.log(`📋 ペット組み合わせを ${allCombos.length} 件読み込みました（5種族 × 4進化段階）`);

  let target;
  if (opts.ids) {
    target = allCombos.filter((c) => opts.ids.includes(c.petId));
  } else if (opts.all) {
    target = allCombos;
  } else {
    const n = opts.count || 2;
    target = allCombos.slice(0, n);
  }
  ...
  const toGenerate = opts.force
    ? target
    : target.filter((combo) => {
        const exists = fs.existsSync(path.join(OUTPUT_DIR, `${combo.petId}.png`));
        if (exists) skipped++;
        return !exists;
      });
  ...
  for (const combo of toGenerate) {
    const { prompt, negativePrompt } = buildPrompt(combo);
    process.stdout.write(`  ${combo.petId} (${combo.nameJp}) ... `);
    try {
      const imageUrl = await generateImage(prompt, negativePrompt);
      const outFile = path.join(OUTPUT_DIR, `${combo.petId}.png`);
      await downloadTo(imageUrl, outFile);
      manifest[combo.petId] = {
        nameJp: combo.nameJp,
        speciesId: combo.speciesId,
        stageId: combo.stageId,
        file: `${combo.petId}.png`,
        prompt,
        provider: 'leonardo',
        modelId: LEONARDO_MODEL_ID,
