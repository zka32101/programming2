...
final userRegisteredAtOverrideProvider =
  ...
  final registeredAtString =
      prefs.getString('kokugo_user_registered_at');
  ...
  await prefs.setString('kokugo_user_registered_at', now.toIso8601String());
  return now;
});

/// kokugo-kore 用：サブスク購読状態プロバイダー
...
/// kokugo-kore 用：クイズアクセス状態を override
...
/// クイズへのアクセス可否（簡略版）
final canAccessKokugoQuizzesProvider = FutureProvider<bool>((ref) async {
  final registeredAt =
      await ref.watch(userRegisteredAtOverrideProvider.future);
  final isSubscribed =
      ref.watch(subscriptionStatusOverrideProvider);
...
/// ペイウォール表示が必要か
final shouldShowKokugoPaywallProvider =
    FutureProvider<bool>((ref) async {
  final canAccess =
      await ref.watch(canAccessKokugoQuizzesProvider.future);
  return !canAccess;
});

/// 無料期間の残り日数
final remainingKokugoFreeDaysProvider =
